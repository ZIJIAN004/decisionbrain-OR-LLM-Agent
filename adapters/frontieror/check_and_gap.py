...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--candidate-root',type=Path,required=True); p.add_argument('--task-root',type=Path,required=True); p.add_argument('--instance-root',type=Path,required=True); p.add_argument('--index',type=Path,required=True); p.add_argument('--output',type=Path,required=True); p.add_argument('--checker-report',type=Path); p.add_argument('--timeout',type=int,default=120)
    ...
    for pid,case in sorted(cases.items()):
        ...
        if prior is not None and prior.get('outcome') != 'feasible':
            rows.append({'paper_id':pid,'outcome':prior.get('outcome'),'error':prior.get('error')}); continue
        if prior is not None and prior.get('outcome') == 'feasible':
            check={'feasible':True}
        elif not inst or not inst.is_file(): rows.append({'paper_id':pid,'outcome':'checker_execution_error','error':'instance missing'}); continue
        else:
            ...
            feasible=check.get('feasible') is True; row={'paper_id':pid,'outcome':'feasible' if feasible else 'infeasible','checker_result':check}

...
# host is unaffected. The scope covers every descendant, so the Gurobi run that
# extract_and_execute_python_code spawns counts against the same cap.
...
def classify(returncode: int, timed_out: bool) -> str:
    ...
    killed by our budget is never scored as OR-LLM-Agent getting it wrong."""
    if timed_out:
        return "task_timeout"
    if returncode == -9:
